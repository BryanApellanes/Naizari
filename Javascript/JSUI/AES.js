...
if (JSUI !== undefined && JSUI !== null) {
    var validateKeySize = function(keySize) {
        if (!(keySize == 128 || keySize == 192 || keySize == 256)) {
            throw new JSUI.Exception("Invalid keysize specified for AES encryption, must be : 128, 192 or 256.");
        }
    }

    JSUI.AES = {};
    JSUI.AES.encrypt = function(plaintext, password, keySize) {
        validateKeySize(keySize);
        return AesCtr.encrypt(plaintext, password, keySize);
    }

    JSUI.AES.decrypt = function(ciphertext, password, keySize) {
        validateKeySize(keySize);
        return AesCtr.decrypt(ciphertext, password, keySize);
    }
